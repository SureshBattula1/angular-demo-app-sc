...
import {
  extractLaravelPaginatorData,
  SmsBulkLogDetailData,
  SmsBulkLogService,
  SmsBulkQueueRow,
  SmsBulkRecipientRow
} from '../../services/sms-bulk-log.service';
...
@Component({
  ...
  templateUrl: './sms-tab-page.component.html',
  styleUrls: ['./sms-tab-page.component.scss']
})
export class SmsTabPageComponent implements OnInit, OnChanges {
  @ViewChild('queueTable') queueTable?: DataTableComponent;

  private logApi = inject(SmsBulkLogService);
  ...
  /** Branches from bulk shell (already loaded) — avoids a duplicate fetch + branch loading spinner. */
  shellBranches: Branch[] = [];

  /** When true, delivery log lists all accessible branches (omit branch filter). */
  @Input() canViewAllBranches = false;

  /** Delivery log (data-table) vs send SMS panel */
  view: 'delivery' | 'send' = 'delivery';

  /** From URL `smsQueue` — when set, show full-page batch recipients (separate from list). */
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(params => {
      ...
      this.applySmsQueryParams(params as Record<string, string | undefined>);
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['branchId']) {
      ...
      // override `smsBranch` from the URL or call `onBranchChanged()` (which strips smsQueue/smsBranch).
      if (this.hasSmsBatchDeepLinkInUrl()) {
        return;
      }
      if (this.canViewAllBranches) {
        return;
  ...
  private hasSmsBatchDeepLinkInUrl(): boolean {
    const qp = this.route.snapshot.queryParams;
    const q = qp['smsQueue'];
    const b = qp['smsBranch'];
  ...
  private applySmsQueryParams(params: Record<string, string | undefined>): void {
    const rawQ = params['smsQueue'];
    if (rawQ != null && rawQ !== '') {
  ...
  private clearBatchDetailQueryParams(): void {
    this.router.navigate([], {
      ...
      queryParams: { smsQueue: null, smsBranch: null },
  ...
  /** After bulk SMS is queued, return to the delivery log list and refresh. */
  onBulkQueued(): void {
    this.clearBatchDetailQueryParams();
    this.batchDetailQueueId = null;
    this.resetDetail();
    ...
    this.logApi
      ...
      .subscribe({
        ...
        error: () => {
          ...
          this.queueRows = [];
          this.tableConfig = { ...this.tableConfig, totalCount: 0 };
          this.snack.open('Could not load SMS send logs.', 'Dismiss', { duration: 4000 });
        }
      });
  }
